...
如果业务仍需要以下能力，应新增边界清晰的类型化 MCP 工具或独立 Plugin，不应把 HTTP/API 细节
放回 PM Agent：
...
原型设计、逆向 PRD、行为基线和经验库也没有进入当前 Plugin。它们不是 E3 底层能力；如果仍有
真实用户需求，应分别评估为独立 Skill 或 Plugin，而不是继续扩大 `oec-pm`。

```mermaid
flowchart LR
    OLD["旧 PM 能力集合"] --> KEEP["保留并强化"]
    OLD --> REMOVE["删除过度编排"]
    OLD --> PLATFORM["平台能力待独立实现"]
