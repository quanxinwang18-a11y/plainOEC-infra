...
旧仓库中的 `script/build-plugin-marketplace.mjs` 读取 Skill、Agent 和 preset registry，
再执行以下转换：
...
- 生成角色到 Skills、Agent groups 和模板的 `plugins/oec-ai/payload/manifest.json`。
...
使用当前 Claude Code 对旧仓库 `plugins/oec-ai/.claude-plugin/plugin.json`
进行实际发现，组件清单是：
...
旧仓库 `plugins/oec-ai/hooks/hooks.json` 中的 SessionStart Hook 在 `startup|clear|compact` 时调用
`plugins/oec-ai/runtime/project-sync.mjs`。同步器只会更新已经存在
...
详见 [E3 平台 3.0.0 真实验收记录](../evidence/e3-platform-3.0.0-real-acceptance.md)。
