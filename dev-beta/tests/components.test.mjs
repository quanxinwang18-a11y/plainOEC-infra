...
test('dev-beta exposes one explicit experimental Skill and no copied platform components', () => {
  const manifest = JSON.parse(readFileSync(resolve(pluginRoot, '.claude-plugin/plugin.json'), 'utf8'));
  assert.equal(manifest.name, 'dev-beta');
  assert.equal(manifest.version, '0.1.0');
  assert.match(manifest.description, /实验性/);
  assert.equal('dependencies' in manifest, false);

  const skill = frontmatter('skills/run-long-coding/SKILL.md');
  assert.equal(skill.metadata.name, 'run-long-coding');
  ...
  assert.doesNotMatch(skill.body, /oec-engineering:run-long-coding/);

  for (const path of ['.mcp.json', 'hooks', 'agents', 'dist', 'bin']) {
    assert.equal(existsSync(resolve(pluginRoot, path)), false, `${path} must not be copied into dev-beta`);
  }
});

test('dev-beta keeps the Codex Skill explicit and carries positive and negative route cases', () => {
  const policy = YAML.parse(readFileSync(resolve(pluginRoot, 'skills/run-long-coding/agents/openai.yaml'), 'utf8'));
  assert.equal(policy.policy.allow_implicit_invocation, false);
  assert.match(policy.interface.default_prompt, /\$run-long-coding/);

  for (const polarity of ['positive', 'negative']) {
    const directory = `evals/run-long-coding-${polarity}`;
    const prompt = frontmatter(`${directory}/prompt.md`);
    const grader = frontmatter(`${directory}/graders/skill-route.md`).metadata;
    assert.equal(prompt.metadata.name, `run-long-coding-${polarity}`);
    assert.ok(prompt.metadata.tags.includes(polarity));
    assert.equal(grader.type, 'tool_used');
    assert.equal(grader.tool, 'Skill');
    assert.match(grader.input_match, /run-long-coding/);
