...
export const ENGINEERING_AGENT_NAMES = ['checker', 'evaluator', 'implementer', 'researcher'];
...
export async function generateEngineeringCodexAgents({ pluginRoot } = {}) {
  const scriptDir = dirname(fileURLToPath(import.meta.url));
  const root = pluginRoot ?? resolve(scriptDir, '..', 'oec-engineering');
...
if (isDirect) {
  const changed = await generateEngineeringCodexAgents();
  process.stdout.write(`Generated ${ENGINEERING_AGENT_NAMES.length} Codex Agents${changed.length ? ` (${changed.join(', ')})` : ' (unchanged)'}\n`);
}
