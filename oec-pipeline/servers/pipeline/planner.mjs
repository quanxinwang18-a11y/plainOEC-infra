...
export class PipelineService {
  ...
  async execute({ planToken }, roots) {
    ...
    const matchingRuns = async () => (await this.client.listRuns(plan.environment, plan.pipeline.id))
      .filter((run) => run.runRemark === `oec-pipeline:${plan.runKey}`);
    const checkpointRun = async (remote, action) => {
      ...
      await atomicJson(path, plan);
      return resultFor(remote, action);
    };
    const recoverExisting = async (action) => {
      const matches = await matchingRuns();
      if (matches.length === 1) return checkpointRun(matches[0], action);
      if (matches.length > 1) {
        plan = {
          ...plan,
          state: 'failed',
    ...
    try {
      ...
      if (deterministic) {
        plan = { ...plan, state: 'failed', error: error.message, failedAt: this.now() };
        await atomicJson(path, plan);
        return { status: 'blocked', errors: [error.message] };
      }
      return { status: 'unknown', planToken, errors: [error.message] };
    }
