...
export const E3_ORIGIN = 'https://one.iflytek.com';
export const CLIENT_ID = 'skill-b';
export const CALLBACK_PORT = 18090;
export const CALLBACK_PATH = '/callback';
export const AUTHORIZE_URL = `${E3_ORIGIN}/login/oauth/authorize/`;
export const TOKEN_URL = `${E3_ORIGIN}/login/oauth/token/`;
...
export function tokenFile(dataDirectory) {
  return join(dataRoot(dataDirectory), 'tokens', 'skill-b.json');
}
...
export async function loadToken(dataDirectory) {
  try {
    return JSON.parse(await readFile(tokenFile(dataDirectory), 'utf8'));
  } catch (error) {
...
export async function saveToken(token, dataDirectory, now = Date.now()) {
  ...
  await atomicWriteJson(tokenFile(dataDirectory), stored);
  return stored;
}

export async function clearToken(dataDirectory) {
  try {
    await unlink(tokenFile(dataDirectory));
  } catch (error) {
...
export function redactSecrets(value, knownSecrets = []) {
  ...
  text = text
    .replace(/("?(?:access_token|refresh_token|acToken)"?\s*[:=]\s*"?)[^"\s,}]+/gi, '$1[REDACTED]')
    .replace(/(Bearer\s+)[A-Za-z0-9._~+\/-]+/gi, '$1[REDACTED]');
  for (const secret of knownSecrets.filter(Boolean)) text = text.replaceAll(String(secret), '[REDACTED]');
  return text;
}

async function tokenRequest(body, fetchFn = fetch) {
...
export function refreshAccessToken(refreshToken, fetchFn = fetch) {
  return tokenRequest({
...
export async function authorizeWithPkce({ fetchFn = fetch, openBrowserFn = openBrowser } = {}) {
  const { verifier, challenge } = generatePkcePair();
  const state = base64Url(randomBytes(32));
  const callback = waitForOAuthCallback(state);
  openBrowserFn(buildAuthorizationUrl({ challenge, state }));
...
export class AuthManager {
  ...
  async getAccessToken({ forceOAuth = false } = {}) {
    const injected = process.env.SKILL_ACCESS_TOKEN;
    if (injected) return { token: injected, source: 'env' };

    if (!forceOAuth) {
      const cached = await loadToken(this.dataDirectory);
      if (cached?.access_token && Number(cached.expires_at) > this.now() + 30_000) {
        return { token: cached.access_token, source: 'local' };
      }
      if (cached?.refresh_token) {
        try {
          const refreshed = await refreshAccessToken(cached.refresh_token, this.fetchFn);
          if (!refreshed.refresh_token) refreshed.refresh_token = cached.refresh_token;
          const stored = await saveToken(refreshed, this.dataDirectory, this.now());
          return { token: stored.access_token, source: 'local' };
        } catch {
          await clearToken(this.dataDirectory);
        }
      }
    }

    const authorized = await this.authorizeFn({ fetchFn: this.fetchFn });
    const stored = await saveToken(authorized, this.dataDirectory, this.now());
    return { token: stored.access_token, source: 'local' };
  }

  async recoverAfter401(source) {
    if (source === 'env') throw new Error('Injected E3 token was rejected; replace SKILL_ACCESS_TOKEN');
    await clearToken(this.dataDirectory);
    return this.getAccessToken({ forceOAuth: true });
  }
}
