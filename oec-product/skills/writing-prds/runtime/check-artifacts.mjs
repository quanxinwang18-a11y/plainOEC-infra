...
// skills/writing-prds/scripts/check-artifacts.mjs
var import_yaml = __toESM(require_dist(), 1);
import { existsSync, readFileSync, readdirSync, realpathSync, statSync } from "node:fs";
import { basename, isAbsolute, join, relative, resolve, sep } from "node:path";
var VERSION_PATTERN = /^v\d+\.\d+\.\d+$/;
var CORE_SECTIONS = ["\u6A21\u5757\u6982\u8FF0", "\u7528\u6237\u6545\u4E8B", "\u9A8C\u6536\u6807\u51C6", "\u5F85\u786E\u8BA4\u4E8B\u9879"];
var TECHNICAL_LANGUAGE = [
...
function detectVersion(workspace) {
  ...
  return readdirSync(directory, { withFileTypes: true }).filter((entry) => entry.isDirectory() && VERSION_PATTERN.test(entry.name)).map((entry) => entry.name).sort(compareVersions).at(-1) ?? null;
}
...
function validateHandoff(workspace, version, incrementModules, errors, warnings) {
  ...
  let handoff;
  try {
    handoff = import_yaml.default.parse(text);
  } catch (error) {
...
function checkArtifacts({ workspace, version, stage = "finalize", strictWarnings = false }) {
...
// skills/writing-prds/scripts/check-artifacts-cli.mjs
var VERSION_PATTERN2 = /^v\d+\.\d+\.\d+$/;
var STAGES = /* @__PURE__ */ new Set(["finalize", "pre-publish"]);
function parseArgs(argv) {
  const args2 = { workspace: process.cwd(), stage: "finalize", json: false, strictWarnings: false };
  for (let index = 0; index < argv.length; index += 1) {
    const token = argv[index];
    if (token === "--workspace") args2.workspace = argv[++index];
    else if (token === "--version") args2.version = argv[++index];
    else if (token === "--stage") args2.stage = argv[++index];
    else if (token === "--json") args2.json = true;
    else if (token === "--strict-warnings") args2.strictWarnings = true;
    else if (token === "--help" || token === "-h") args2.help = true;
    else throw new Error(`Unknown argument: ${token}`);
  }
  if (!STAGES.has(args2.stage)) throw new Error(`Invalid --stage: ${args2.stage}`);
  if (!args2.workspace) throw new Error("--workspace requires a value");
  if (args2.version && !VERSION_PATTERN2.test(args2.version)) throw new Error(`Invalid --version: ${args2.version}`);
  return args2;
}
function usage() {
