...
async function devFixture(productRoot) {
  ...
  await writeFiles(dev, {
    'ai-docs/Spec/README.md': '# Engineering\n',
    'ai-docs/Spec/specs/payment.md': `---
...
test('a legacy ID matching both canonical locations is rejected as ambiguous', async () => {
  const dev = await devFixture(null);
  await writeFiles(dev, {
    'ai-docs/Spec/changes/v1.2.3-paymentRetry/change.md': '# duplicate location\n',
  });
  const result = await resolveTaskRef({ devRoot: dev, changeId: 'v1.2.3-paymentRetry' });
...
test('versioned and unversioned task references do not share path semantics', async () => {
  ...
  await writeFiles(dev, {
    'ai-docs/Spec/changes/2026-09-02-cache-fix/change.md': `---
...
`,
  ...
  assert.match(result.relativePath, /ai-docs\/Spec\/changes\/2026-09-02-cache-fix/);
});
...
test('Spec reminder is advisory, deterministic, and read-only', async () => {
  const dev = await devFixture(null);
  const before = await readFile(join(dev, 'ai-docs/Spec/specs/payment.md'), 'utf8');
  ...
  assert.equal(await readFile(join(dev, 'ai-docs/Spec/specs/payment.md'), 'utf8'), before);
});
...
test('invalid module index blocks task readiness', async () => {
  const dev = await devFixture(null);
  await writeFiles(dev, {
    'ai-docs/Spec/module-index.yaml': `schema_version: 2
modules:
  - id: Payment
`,
  });
