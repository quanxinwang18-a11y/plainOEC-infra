...
var CHANGE_PATH = new RegExp(`^ai-docs\\/Spec\\/changes\\/(${CHANGE_ID_PATTERN})$`);
...
function relativeTaskPath(parsed) {
  ...
  if (parsed.kind === "change") return `ai-docs/Spec/changes/${parsed.changeId}`;
...
async function findLegacyId(devRoot, legacyChangeId) {
  const candidates = [];
  const directRelative = `ai-docs/Spec/changes/${legacyChangeId}`;
...
async function loadEngineeringReferences(root, warnings) {
  const engineeringRoot = join2(root, "ai-docs", "Spec");
...
async function checkModuleIds(root, modules, errors, warnings) {
  const indexPath = join2(root, "ai-docs", "Spec", "module-index.yaml");
  ...
  if (await pathExists(join2(root, "ai-docs", "Spec", "specs"))) {
...
async function loadSpecs(devRoot) {
  const specRoot = resolve4(devRoot, "ai-docs", "Spec", "specs");
...
async function loadModuleIndex(devRoot) {
  const path = resolve4(devRoot, "ai-docs", "Spec", "module-index.yaml");
  try {
    ...
    return { modules: [], warnings: [issue("module-index-invalid", "ai-docs/Spec/module-index.yaml", error.message, "warning")] };
  }
}
...
async function findSpecReminders({
  ...
  if (uncoveredPaths.length > 0 && (high || specs.length === 0)) {
    addCandidate(candidates, {
      kind: "create-spec",
      target: "ai-docs/Spec/specs/<module>.md",
      severity,
  ...
  if (high && allPaths.length > 0) {
    ...
    if (boundaryPaths.length > 0 || explicitSignals.some((signal) => ["boundary", "ownership", "compatibility"].includes(signal))) {
      addCandidate(candidates, {
        kind: "review-adr",
        target: "ai-docs/Spec/decisions/ADR-NNNN-<slug>.md",
        severity: "review",
...
async function loadModuleIndex2(engineeringRoot, specIds, errors) {
  ...
  try {
    ...
    errors.push(issue2("module-index-read-failed", "ai-docs/Spec/module-index.yaml", error.message));
    return [];
  }
  if (info.isSymbolicLink() || !info.isFile()) {
    errors.push(issue2("module-index-invalid", "ai-docs/Spec/module-index.yaml", "module-index.yaml must be a regular file"));
    return [];
  }
  let document;
  try {
    ...
    errors.push(issue2("module-index-invalid", "ai-docs/Spec/module-index.yaml", error.message));
    return [];
  }
  if (!document || typeof document !== "object" || Array.isArray(document)) {
    errors.push(issue2("module-index-invalid", "ai-docs/Spec/module-index.yaml", "module index must be a YAML mapping"));
    return [];
  }
  if (document.schema_version !== 1) {
    errors.push(issue2("module-index-schema", "ai-docs/Spec/module-index.yaml", "schema_version must be 1"));
  }
  if (!Array.isArray(document.modules) || document.modules.length === 0) {
    errors.push(issue2("module-index-empty", "ai-docs/Spec/module-index.yaml", "modules must be a non-empty array"));
    return [];
  }
  ...
  for (const item of document.modules) {
    ...
    if (typeof id !== "string" || !MODULE_ID2.test(id)) {
      errors.push(issue2("module-id-invalid", "ai-docs/Spec/module-index.yaml", "module id must match [a-z][a-z0-9-]*"));
      continue;
    }
    if (ids.has(id)) {
      errors.push(issue2("module-id-duplicate", "ai-docs/Spec/module-index.yaml", `${id} is already declared`));
      continue;
    }
    ids.set(id, true);
    ...
    for (const glob of paths) {
      ...
      if (problem) errors.push(issue2("module-glob-invalid", "ai-docs/Spec/module-index.yaml", `${glob}: ${problem}`));
    }
    const specs = validateStringList(item.specs, "specs", "ai-docs/Spec/module-index.yaml", errors, { pattern: SPEC_ID2 });
    for (const spec of specs) {
      if (!specIds.has(spec)) errors.push(issue2("module-spec-reference-missing", "ai-docs/Spec/module-index.yaml", `${id} references unknown Spec: ${spec}`));
    }
    const dependsOn = validateStringList(item.depends_on, "depends_on", "ai-docs/Spec/module-index.yaml", errors, { pattern: MODULE_ID2 });
    modules.push({ id, title: item.title, owner: item.owner, paths, specs, dependsOn });
  }
  for (const module of modules) {
    for (const dependency of module.dependsOn) {
      if (dependency === module.id) errors.push(issue2("module-self-dependency", "ai-docs/Spec/module-index.yaml", `${module.id} may not depend on itself`));
      else if (!ids.has(dependency)) errors.push(issue2("module-dependency-missing", "ai-docs/Spec/module-index.yaml", `${module.id} depends on unknown module: ${dependency}`));
    }
  }
  return modules;
}
async function loadEngineering(root, roots = { devRoot: root, productRoot: null }, { validateSources = true, requireRoot = true } = {}) {
  ...
  if (!await exists(indexPath)) {
    errors.push(issue2("engineering-index-missing", "ai-docs/Spec/README.md", "engineering index is required"));
  }
  ...
  for (const absolute of markdownFiles) {
    ...
    for (const match of text.matchAll(/!?\[[^\]]*\]\(([^)]+)\)/g)) {
      ...
      if (!isInside2(engineeringRoot, target)) {
        errors.push(issue2("link-path-escape", path, `link escapes ai-docs/Spec: ${targetText}`));
        ...
        if (!isInside2(engineeringRoot, resolved)) {
          errors.push(issue2("link-path-escape", path, `link resolves outside ai-docs/Spec: ${targetText}`));
        }
      }
    }
  }
...
async function checkTeamSpecs({ workspace, devRoot, productRoot, change } = {}) {
  ...
  if (change && !result.changes.some((item) => item.id === change)) {
    result.errors.push(issue2("change-not-found", `ai-docs/Spec/changes/${change}`, "requested change was not found"));
  }
  return {
