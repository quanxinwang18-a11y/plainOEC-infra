---
name: code-plan
description: Use as the first planning step when the user wants to develop a non-trivial change from a PRD, Story, HANDOFF, issue, or other requirement; establish confirmed scope, acceptance, technical direction, and a safe code boundary without requiring the user to know internal task documents. Do not use for ordinary implementation, a small obvious fix, product authoring, or external task creation.
...
ready check hands control back to the main session to invoke `code-implement`; do not make the user
