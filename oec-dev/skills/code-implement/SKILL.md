---
name: code-implement
description: Continues planned development in the Main Session when the user wants to implement an existing ready task or resume after confirmed requirement planning. The user need not repeat an internal task identity resolved by the conversation. Do not use to plan a PRD-only request, create task documents, handle a small direct fix, review or close work, or operate external platforms.
...
- Do not commit, push, merge, call `code-finish`, create E3 tasks, update E3/Pipeline, or deploy.
...
TDD request, `debug` when the root cause is unclear, and `code-review` only when the user
