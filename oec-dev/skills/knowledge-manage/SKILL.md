...
description: Helps a team establish, preserve, reconcile, or refresh durable engineering knowledge when the user wants to understand current architecture, record a lasting decision, initialize team context, or revisit knowledge after a change. It may maintain Specs, ADRs, or stale-knowledge reminders. Do not use for one-off task planning, ordinary implementation, legacy migration, or product requirements.
...
one-time task `spec.md`/`design.md`; those belong to `code-plan`. The repository fact source for the
...
create state, or block ordinary coding in reminder mode. `code-plan`, `code-review`, and
`code-finish` may run this read-only check at their natural checkpoints.
