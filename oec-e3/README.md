...
The current migration stage exposes the four existing PRD publication tools plus guarded planning,
selection, and execution for development tasks. The Plugin is not added to the Marketplace until
the Product dependency cutover, so normal installations cannot load both the legacy embedded Server
and this Server.
