...
Reuse the host-discovered `task-implementer`, `web-evaluator`, `change-checker`, and `oec-spec` when they are available.
...
session to use `change-plan`. Do not invent or create product scope inside this Skill. Ambiguous
...
1. Dispatch the host `task-implementer` once with the canonical taskRef, checklist, and explicit reference paths. Retain the returned Agent ID.
2. Dispatch the host `web-evaluator` once with the same inputs, target, changed paths, and observed results. Retain that Agent ID.
3. On a runtime failure, forward complete structured findings through the main session to the same implementation Agent. Do not summarize them or forward task-implementer self-assessment to web-evaluator.
...
After runtime evaluation passes, dispatch the host `change-checker`. Stop on judgment issues. If the check
changes code, resume the web-evaluator for a complete runtime recheck if the budget permits.
...
task closure; the user must separately decide whether to invoke `change-close`.
