...
### 3. Generate
Write `versions/v{x.y.z}/prd/prd-v{x.y.z}.md`. One file per version, multiple modules in one file. 11-section structure per module (see `templates/increment-prd.md`). Product language throughout (see `references/forbidden-terms.md`). User stories in GWT format (see `references/user-story-gwt.md`). No invention, no half-finished work.
...
### 6. Finalize
Merge into `prd-all.md` (increment wins on conflict, keep base where silent, insert new). Append to changelog (see `references/changelog-format.md`). Run `scripts/product-flow-gate.mjs --stage finalize`. For legacy baseline, initialize from increment.
...
### 2. Edit
Modify `prd-all.md` directly. Parse `git diff`, draft summary, get PM confirmation, append to changelog (see `references/changelog-format.md`).
