...
async function loadEngineeringReferences(root, warnings) {
  const engineeringRoot = join(root, 'ai-docs', 'engineering');
...
async function checkModuleIds(root, modules, errors, warnings) {
  const indexPath = join(root, 'ai-docs', 'engineering', 'module-index.yaml');
  ...
  if (await pathExists(join(root, 'ai-docs', 'engineering', 'specs'))) {
...
/**
 * Validate the task-level Spec and Design pair. The checker is independent of the team-Spec root;
