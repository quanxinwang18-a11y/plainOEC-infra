...
one-time task `spec.md`/`design.md`; those belong to `plan-change`. The repository fact source for the
...
create state, or block ordinary coding in reminder mode. `plan-change`, `review-code`, and
`close-change` may run this read-only check at their natural checkpoints.
