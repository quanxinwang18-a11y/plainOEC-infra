...
argument-hint: "[existing change ID or currently confirmed change]"
...
1. Dispatch `implementer` once with the change ID, completion checklist, and explicit reference
   paths. Retain the returned Agent ID.
2. Dispatch `evaluator` once with the same inputs, the running target, changed paths, and observed
...
changes code, resume `evaluator` for a complete runtime recheck; if that fails and budget remains,
resume `implementer` again.
...
change closure; `close-change` remains a separate user action.
