...
`oec-engineering` 是稳定的 OEC Dev Plugin，提供任务级工程执行、模块上下文、Team Specs、技术决策、
故障诊断和代码评审能力。它不是固定的 Dev workflow engine；普通编码仍由 Main Session 完成。

长时 Web/full-stack 编码和 Playwright 多轮编排已经移到独立的实验性 `dev-beta` Plugin，不属于本
Plugin 的默认流程。
...
`dev-beta` 要求先具备本 Plugin 的 Agent 和 `oec-spec`；它不会复制这些文件或 runtime。
...
/oec-engineering:develop-change
...
`plan-change`、`review-code` 或 `close-change`。

### 其他 Skills

- `manage-specs`：维护有代码证据支持的 current-state Specs、ADR 和必要的 change package；`remind`
  模式只读。
- `migrate-legacy-ai-docs`：将旧 `ai-docs` 中仍有效的工程事实迁移到当前结构，保留原文件。
- `plan-change`：为 PRD、Story、HANDOFF、issue 或非平凡变更创建任务级 `spec.md` 和 `design.md`。
- `challenge-decision`：在规划或实现前压力测试技术决策。
- `prototype-decision`：用 throwaway artifact 回答一个交互、行为或状态设计问题。
- `develop-test-first`：只在用户明确要求 TDD/test-first 时采用 red-green-refactor。
- `diagnose-failure`：处理难复现、flaky、性能回退或根因不清的问题。
- `review-code`：只读、风险优先地评审 diff、commit、branch 或 PR。
- `close-change`：检查最终证据、收口 Team Specs/ADR，并在用户确认后提交精确路径。

文档、Team Specs、迁移目标和提交都遵循“展示精确路径 → 用户确认”的边界；`develop-change` 只在用户
...
```text
oec-engineering/agents/*.md
```

实验性的 Codex 镜像位于：

```text
oec-engineering/.codex-plugin/agents/*.toml
```

镜像由构建生成，不要手工编辑：

```bash
npm run generate:engineering-agents
```
...
→ 选择相关 Specs/ADR
→ plan-change 创建 spec.md + design.md
→ develop-change 或 Main Session 实现
→ 测试、typecheck、lint
→ review-code / checker（按需）
→ close-change（用户需要时）
```
...
```text
/dev-beta:run-long-coding [existing taskRef]
```

## 迁移旧项目

显式或自然语言请求迁移时，`migrate-legacy-ai-docs` 会先运行只读 legacy audit，提出“源路径 → 分类
→ 目标路径 → 证据”的计划，保留旧文件，只在确认后写入 `ai-docs/engineering/`。它不删除 `.oec-ai`、
旧 `.claude`/`.codex`、E3 mapping 或历史文件。
